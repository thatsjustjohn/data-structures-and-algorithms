...
const hasChildrenEntries = (arr, character) => {
  for (let index = 0; index < arr.length; index++) {
    if(Object.entries(arr[index])[0][1] === character){
      let tempBool = Object.entries(arr[index])[2][1].length === 0 ? false : true;
      return tempBool;
    }
  }
  return false;
...
const totalCharacters = (arr) => {
  let total = 0;
  for (let index = 0; index < arr.length; index++) {
    const obValues = Object.values(arr[index]);
    if(obValues[0]) total++;
    if(obValues[1]) total++;
    if(obValues[2]) total += obValues[2].length;
  }
  return total;
};
