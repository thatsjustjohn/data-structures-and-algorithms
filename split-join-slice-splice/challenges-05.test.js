...
const howMuchPencil = (str) => {
  ...
  for (let index = 0; index < lengthOfString+1; index++) {
...
const listFoods = (recipe) => {
  let result = [];
  const lengthOf = recipe.ingredients.length;
  for (let index = 0; index < lengthOf; index++) {
    const firstSpace = recipe.ingredients[index].indexOf(' ') + 1;
    result.push(recipe.ingredients[index].slice(recipe.ingredients[index].indexOf(' ', firstSpace)+1));
  }
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  const lengthOf = recipe.steps.length;
  for (let index = 0; index < lengthOf; index++) {
    result.push(recipe.steps[index].split(' ')[0]);
  }
  return result;
};
