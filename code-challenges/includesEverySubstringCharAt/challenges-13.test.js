...
const sortByDay = (arr) => {
  return daysOfWeek.reduce((accumulator, currentValue) => {
    accumulator.push(arr.reduce((accumulatorSub, currentValueSub) => {
      if(currentValueSub.includes(currentValue)){
        accumulatorSub.push(currentValueSub);
      }
      return accumulatorSub;
    }, []));
...
const characterByIndex = (arr) => arr.reduce((accumulator, currentValue, index) => accumulator.concat([currentValue.charAt(index)]), []);
